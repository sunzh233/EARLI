...
DEPOT_LOCATION = 0
STATE_PROPERTIES = {'head', 'demand', 'capacity', 'current_time'}
...
class VRPTW(RoutingBase):
    def __init__(self, config, **kwargs):
        GymEnv.__init__(self)
        state_properties = list(STATE_PROPERTIES)
        problem_properties = ['demand', 'capacity', 't_min', 't_max', 'dt']
        prop_types = dict(demand=torch.float32, capacity=torch.float32, t_min=torch.float32, t_max=torch.float32, dt=torch.float32)
        ...
        # print(f'Vehicle penalty: {self.extra_car_penalty}; Capacity penalty: {self.unused_capacity_penalty}; Time penalty: {self.time_penalty}')
    ...
    def set_spaces(self):
        ...
        # Time windows (static)
        self.t_min = torch.zeros(self.maybe_extend([self.n_parallel_problems, self.problem_size]), device=self.device)
        self.t_max = torch.zeros(self.maybe_extend([self.n_parallel_problems, self.problem_size]), device=self.device)
        self.service_time = torch.zeros(self.maybe_extend([self.n_parallel_problems, self.problem_size]), device=self.device)
        ...
        self.current_time = torch.zeros(size=self.maybe_extend([self.n_parallel_problems, 1]), device=self.device)
        ...
        n_node_features += 3  # time windows: t_min, t_max, service_time
        ...
        # Node features: depot/non depot, demand, head position, masked / unmasked, time windows
        if self.stable_baselines_compatibility:
            ...
            self.observation_space = Dict(
                    {'loc'             : Box(low=-np.inf, high=np.inf, shape=(self.problem_size, 2)),
                     ...
                     't_min'           : Box(low=0, high=np.inf, shape=(self.problem_size, 1)),
                     't_max'           : Box(low=0, high=np.inf, shape=(self.problem_size, 1)),
                     'service_time'    : Box(low=0, high=np.inf, shape=(self.problem_size, 1)),
                    ...
                    'dt'              : Box(low=0, high=np.inf, shape=(self.problem_size, 1)),
                     'current_time'    : Box(low=0, high=np.inf, shape=(1,)),
            ...
            self.observation_space = Dict(
                    {'loc'             : Box(low=-np.inf, high=np.inf, shape=(*self.batch_dim, self.problem_size, 2)),
                     ...
                     't_min'           : Box(low=0, high=np.inf, shape=(*self.batch_dim, self.problem_size, 1)),
                     't_max'           : Box(low=0, high=np.inf, shape=(*self.batch_dim, self.problem_size, 1)),
                     'service_time'    : Box(low=0, high=np.inf, shape=(*self.batch_dim, self.problem_size, 1)),
                     ...
                     'current_time'    : Box(low=0, high=np.inf, shape=(*self.batch_dim, 1)),
    ...
    def reset_static_properties(self, n_problems_to_reset, copy_all, indices):
        ...
        if self.extended_output_format:
            tmin = (self.reference['t_min'][fixed_dataset_indices].unsqueeze(1).clone()
                .expand_as(self.t_min[indices]))
            tmax = (self.reference['t_max'][fixed_dataset_indices].unsqueeze(1).clone()
                .expand_as(self.t_max[indices]))
            svc = (self.reference['dt'][fixed_dataset_indices].unsqueeze(1).clone()
               .expand_as(self.service_time[indices]))
            self.t_min[indices] = tmin
            self.t_max[indices] = tmax
            self.service_time[indices] = svc
        else:
            self.t_min[indices] = self.reference['t_min'][fixed_dataset_indices].clone()
            self.t_max[indices] = self.reference['t_max'][fixed_dataset_indices].clone()
            self.service_time[indices] = self.reference['dt'][fixed_dataset_indices].clone()
        ...
        if self.normalize:
            ...
            # Normalize time windows if needed
            self.t_min[indices] = self.t_min[indices] * self.reward_normalization
            self.t_max[indices] = self.t_max[indices] * self.reward_normalization
            self.service_time[indices] = self.service_time[indices] * self.reward_normalization
    ...
    def _step(self, actions, calc_obs=True):
        ...
        t_min = self.t_min.view(-1, self.problem_size)
        t_max = self.t_max.view(-1, self.problem_size)
        service_time = self.service_time.view(-1, self.problem_size)
        ...
        # Time update: travel time + service time
        travel_time = distance_matrix[dummy_ind, head, visit_site]
        arrival_time = current_time + travel_time
        # Wait if arrive early
        start_service_time = torch.max(arrival_time, t_min[dummy_ind, visit_site])
        # Update current time after service
        current_time[active_env] = start_service_time[active_env] + service_time[dummy_ind, visit_site][active_env]
        ...
        # Time penalty: early arrival (wait time) or late arrival
        early_penalty = torch.max(t_min[dummy_ind, visit_site] - arrival_time, torch.zeros_like(arrival_time))
        late_penalty = torch.max(arrival_time - t_max[dummy_ind, visit_site], torch.zeros_like(arrival_time))
        time_penalty = early_penalty + late_penalty
        reward -= self.time_penalty * time_penalty
    ...
    def get_pos_representation(self, indices=None, extra_global_features=None):
        ...
        obs = {
            'loc'             : self.pos[indices],  # todo: is clone neccessary?
            ...
            't_min'           : self.t_min[indices].unsqueeze(-1).clone(),
            't_max'           : self.t_max[indices].unsqueeze(-1).clone(),
            'service_time'    : self.service_time[indices].unsqueeze(-1).clone(),
            'current_time'    : self.current_time[indices].clone(),
        ...
        # Provide alternative key names expected by embeddings/encoders
        # some init embeddings expect 'tmin'/'tmax'/'dt' (no underscore), add aliases
        obs['tmin'] = obs['t_min']
        obs['tmax'] = obs['t_max']
        obs['dt'] = obs['service_time']

        if self.config['system']['use_tensordict']:
            obs = TensorDict(obs, batch_size=batch_dim)
            if not self.config['system']['save_obs_on_gpu']:
                obs = obs.to('cpu')
            # also ensure alias keys are present inside the TensorDict object
            try:
                obs['tmin'] = obs['t_min']
                obs['tmax'] = obs['t_max']
                obs['dt'] = obs['service_time']
            except Exception:
                # if assignment fails for any reason, continue without raising here
                pass
        return obs

    def state_to_hashable(self, state):
        return state['head'].item(), tuple(state['demand'].tolist()), np.round(state['capacity'].item(), 6), np.round(state['current_time'].item(), 6)
