...
print("[Schedule] stage | total_steps | n_steps | lr | epochs | est_minutes")
total_sec = 0.0
for s, t, ns, lr, ep, c in zip(stages, steps, n_steps, lrs, epochs, sec_per_step):
    est = t * c
    total_sec += est
    print(f"[Schedule] {s:>4} | {t:>10} | {ns:>7} | {lr:>8.2e} | {ep:>6} | {est/60:>10.1f}")
...
PY
  ...
  echo "[2/3][n=${SIZE}] total_steps=${TOTAL_STEPS}, n_steps=${N_STEPS}, lr=${STAGE_LR}, epochs=${STAGE_EPOCH}, batch_size=${STAGE_BATCH_SIZE}"
